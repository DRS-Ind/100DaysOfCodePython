...
class PasswordManager:
    ...
    def save_to_file(self) -> None:
        ...
        if len(website.get()) == 0 or len(email.get()) == 0 or len(password.get()) == 0:
            ...
            if to_save:  # if user agree save data to the file
                with open(file="password_info.txt", mode="a") as file:
                    file.write(" | ".join([website.get(), email.get(), password.get()]) + "\n")
    ...
    def start(self) -> None:
        ...
        self.website_text.grid(row=1, column=1, columnspan=2, sticky="EW")
