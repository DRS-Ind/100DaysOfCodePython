...
class NotificationManager:
    ...
    def __init__(self):
        ...
        self.company_email = os.environ["COMPANY_EMAIL"]
    ...
    def send_emails(self, emails: list, message_body: str) -> None:
        """
        The function sends emails with a flight deal for all emails in the mailing list.

        :param emails: mailing list
        :param message_body: message to send
        """
        for email in emails:
            with smtplib.SMTP("smtp.gmail.com") as connection:
                connection.starttls()
                connection.login(user=self.company_email, password=os.environ["EMAIL_PASSWORD"])
                connection.sendmail(from_addr=self.company_email, to_addrs=email,
                                    msg=f"Subject:New cheap deal\n\n{message_body}")
